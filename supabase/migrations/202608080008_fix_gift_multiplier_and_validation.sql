...
BEGIN
  ...
  -- Multiplier cannot exceed recipient count, and maximum is 100
  v_effective_multiplier := COALESCE(p_combo_count, v_receivers_count);

  IF v_effective_multiplier > v_receivers_count THEN
    RAISE EXCEPTION 'Invalid gift multiplier: % exceeds selected recipient count %.', v_effective_multiplier, v_receivers_count;
  END IF;

  IF v_effective_multiplier > 100 THEN
    RAISE EXCEPTION 'Invalid gift multiplier: % exceeds maximum allowed 100x.', v_effective_multiplier;
  END IF;

  IF v_effective_multiplier < 1 THEN
    RAISE EXCEPTION 'Invalid gift multiplier: must be at least 1x.';
  END IF;
