-- Migration: 202608070022_fix_gift_transactions_table_name.sql
-- Description: Fix table name error in send_star_gift RPC by inserting into public.gift_transactions and public.gift_history.
...
begin
  ...
  foreach v_receiver_id in array p_receiver_ids loop
    ...
    insert into public.gift_transactions (
      ...
      p_room_id, v_sender_id, v_receiver_id, p_gift_id, coalesce(v_gift_record.name, 'Gift'), coalesce(to_jsonb(v_gift_record)->>'icon', to_jsonb(v_gift_record)->>'icon_url', ''),
