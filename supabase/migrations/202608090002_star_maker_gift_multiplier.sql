-- Migration: 202608090002_star_maker_gift_multiplier.sql
-- Description: StarMaker Gift Multiplier logic where Total Gifts = Selected Users × Multiplier.
-- Multiplier applies to EACH selected recipient. Cost, balance deduction, seat gems, AP, daily tasks, and room totals update according to actual total gift quantity.
...
BEGIN
  ...
  -- Multiplier Calculation (StarMaker Rule):
  -- Multiplier applies to EACH selected recipient.
  -- Total Gifts = Selected Recipients x Multiplier
  v_effective_multiplier := COALESCE(p_combo_count, 1);
  IF v_effective_multiplier < 1 THEN
    v_effective_multiplier := 1;
  END IF;
  IF v_effective_multiplier > 100 THEN
    v_effective_multiplier := 100;
  END IF;
  ...
  -- Gems per recipient = multiplier x gem unit value
  v_single_receiver_gems := v_gem_unit_value * v_effective_multiplier;

  -- Total room gems = total quantity x gem unit value
